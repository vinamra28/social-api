...
// @route    GET api/profile/me/:company
// @desc     Get current users profile based on company name
// @access   Private
router.get('/me/:company', auth, async (req, res) => {
  try {
    const profile = await Profile.findOne({ company: req.params.company }).populate(
      'user',
      ['name', 'avatar']
    );
